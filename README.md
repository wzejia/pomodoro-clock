...
- **迷你胶囊悬浮窗**：无边框、置顶、可拖拽；倒计时 + 阶段色标签 + 播放/暂停；底部 2px 细进度条实时显示阶段进度
- **单击展开统计面板**：开始/暂停/重置/跳过/切换阶段；今日番茄、专注分钟、连续打卡天数；本周每日柱状图（含每日目标虚线参考）；月历热力图（可按月翻看）
- **阶段自动流转**：工作 → 短休 → 工作……每 4 个番茄进入长休（间隔可调）；支持自动开始下一阶段；完成时系统通知 + 轻提示音
- **托盘驻留**（v1.0.1）：关闭窗口 = 隐藏到托盘继续计时；自绘苹果风托盘菜单（暂停/重置/设置/退出，动态文案跟随状态）；支持开机自启；单实例运行（二启自动聚焦已有窗口）
- **全局热键**（v1.0.1）：默认 `Ctrl+Alt+O` 随时唤回/暂停；可在设置中自定义改键（按下即录、占用回退保旧键、可一键关闭）
- **双主题 × 双材质**：浅色/深色主题，经典/液态玻璃材质自由组合
- **可配置**：工作/短休/长休时长、长休间隔、自动开始下一阶段、每日目标个数（1–20），全部持久化到 `config.json`
- **三阶段自定义配色**（v1.0.2）：专注/休息/长休各自独立配色，自绘 iOS 风色盘（饱和度×明度方块 + 彩虹色相条 + HEX 色号输入），改色即时生效
- **统计本地存储**：`stats.json` 存 exe 同目录，无云端、无账号
- **v17c 动效**（v1.0.2）：面板展开/收起丝滑滑停，零回弹，数字 FLIP 翻页动画
...
测试：`cd src-tauri && cargo test`（50 个，全绿基线）
...
| `src-tauri/src/stats.rs` | 记录、聚合（今日/本周/连签/月历）、JSON 存取 |
| `src-tauri/src/appconfig.rs` | 配置读写（时长/热键/主题/材质/自启/配色） |
| `src-tauri/src/lib.rs` | IPC 命令、tick 线程、通知、托盘、单实例、动效 |
